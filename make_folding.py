...
# sample target
def factorize_mvau_folding_product(emb_dim, mlp_dim, seq_len, num_classes):
    ...
    def common_divisor(n):
        return (emb_dim % n) == (mlp_dim % n) == (num_classes % n) == 0
    ...
    # common divisor.
    return 1, math.gcd(emb_dim, mlp_dim, num_classes)


# Generates a folding configuration for a transformer model of multiple layers
# each with multiple heads
def make_folding(
        num_heads, num_layers, emb_dim, mlp_dim, seq_len, num_classes, **_
):
...
if __name__ == "__main__":
    ...
    folding = make_folding(**params["model"], seq_len=eval_data[0][0].shape[0])
    # Dump the folding dictionary as json
    with open(params["build"]["finn"]["folding_config_file"], "w") as file:
