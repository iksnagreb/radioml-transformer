...
# Range analysis for MatMul cannot handle a bias, so we need to move Add behind
# the MatMul
...
SUPPORTED_MONOTONIC_ACTIVATIONS = {
    # TODO: FINN already supports Identity and ReLU quantization to
    #  multi-threshold conversion, with a potential bug in the conversion below
    #  it is better to disable these for now...
    "Identity",
    "Relu",
    "LeakyRelu",
    "Clip",
...
def match_quant_monotonic_activation(node: NodeProto, model: ModelWrapper):
    ...
    # TODO: Add BipolarQuant currently still missing full RA support
    # TODO: There seems to be a bug in handling standalone quantizers, as FINN
    #  already has well-tested support disable this for now...
...
class QuantActivationToMultiThreshold(Transformation):
    ...
    def apply(self, model: ModelWrapper):  # noqa
        ...
        # Remove identities such as *1 or +0, especially almost zero values (due
        # to rounding, e.g., 1e-5 or smaller) might trigger some misbehavior of
        # the range analysis which would for example refuse some integer range
        # propagation of such almost zero biases.
        model = model.transform(RemoveIdentityOps())
        # Move potential scalar bias past the MatMul operator
        ...
        for index, node in enumerate(graph.node):
            ...
            if inp in range_info and out in range_info:
                ...
                y0 = - 2 ** (bits - int(signed))
                y1 = 2 ** (bits - int(signed)) - 1
                ...
                max_inp = range_info[inp].range[1]

                # # Check whether this is a signed quantizer
                # signed = getCustomOp(quant).get_nodeattr("signed")
                # # Padding from the left accounts for the difference between the
                # # minimum output produced by the threshold function and the
                # # lower bound from the annotated range information
                # paddingl = int(y0.min() + 2 ** (bits - int(signed)))
                # # Padding from the right accounts for the difference between the
                # # maximum output produced by the threshold function and the
                # # upper bound from the annotated range information
                # paddingr = int(2 ** bits - 1 - len(thresholds) - paddingl)
                #
                # model.save("debug.onnx")
                # # Padding must make sense...
                # assert paddingl >= 0, f"Negative padding for {quant.name}"
                # assert paddingr >= 0, f"Negative padding for {quant.name}"
                # assert (len(thresholds) + paddingl + paddingr) == 2 ** bits - 1, f"{len(thresholds)=}, {paddingl=}, {paddingr=}, {2**bits-1}, {quant.name=}, {bits=}, {y0.min()=}, {y1.max()=}"

                # # Fill up thresholds from the left and right repeating the lower
                # # and upper bound of the input range
                ...
                # Bias correction
                # bias += (scale * (y0.min() - float(- 2 ** (bits - 1) if signed else 0) - padding)).astype(np.float32)
                # bias += (scale * (- padding)).astype(np.float32)
                # bias += scale * (y0.min() - range_info[out].int_range[0])
